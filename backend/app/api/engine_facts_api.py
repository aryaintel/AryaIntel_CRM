...
def _resolve_latest_run_id(conn: sqlite3.Connection, scenario_id: int, category: Optional[str], sheet: Optional[str]) -> Optional[int]:
    ...
    if category:
        sql.append("AND category_code=?"); args.append(category)
    if sheet:
        sql.append("AND sheet_code=?"); args.append(sheet)
...
def get_engine_facts(
    ...
    Notes:
    - If latest=true and no persisted run exists for the scope, returns 404 (FE should treat as empty for that sheet).
    - Otherwise, empty result returns 200 with rows=[].
    """
    # Unify aliases
    ...
    try:
        ...
        if effective_run_id is None and latest:
            ...
            if effective_run_id is None:
                # Keep 404 behavior only for 'latest' resolution per sheet/category scope
        ...
        if series:
            ...
            if len(series_list) == 1:
                wh.append("series=?"); args.append(series_list[0])
            elif len(series_list) > 1:
                placeholders = ",".join(["?"] * len(series_list))
                wh.append(f"series IN ({placeholders})"); args.extend(series_list)
