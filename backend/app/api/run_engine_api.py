# relative path: backend/app/api/run_engine_api.py
# DB standard: backend/app.db (no legacy fallback)
...
def _persist_results(db: Session, scenario_id: int, req: RunEngineRequest, generated: List[SheetPayload]) -> Tuple[int, Optional[int]]:
    ...
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # CRITICAL FIX: commit the transaction so that run_id & facts persist
    db.commit()
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
def _db_path() -> str:
    """Project standard only (no fallback)."""
...
@router.get("/engine/last-run")
def get_last_run(scenario_id: int = Query(..., ge=1)):
    """
    Diagnostic: returns latest run_id and row count from engine_facts_monthly for given scenario.
    Uses the single canonical DB path (backend/app.db).
    """
    db_path = _db_path()
    ...
    try:
        rid = conn.execute(
            "SELECT MAX(id) AS rid FROM engine_runs WHERE scenario_id=?",
            (scenario_id,),
        ).fetchone()["rid"]
        count = None
        if rid is not None:
        ...
        return {"db_path": db_path, "scenario_id": scenario_id, "latest_run_id": rid, "rows": count}
    finally:
        conn.close()
