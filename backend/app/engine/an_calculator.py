# backend/app/engine/an_calculator.py
"""
AN Engine — R+F index-based monthly facts with Freight & Mgmt Fee split
----------------------------------------------------------------------

Bu modül AN kategorisi için aylık Revenue/COGS/GP hesaplar ve Excel sayfalarıyla
paralel veri üretir:
  - c.Sales-AN (aylık, revenue bileşenleri)
  - oQ.Finance-AN (çeyreklik agregasyon)
  - oA.Finance-AN (yıllık agregasyon)

Hesap saf, kalıcılaştırma ince tutulmuştur. engine_facts_monthly tablosunda
`series` kolonu (migration 20251016) beklenir.
...
# series-aware persist
from app.engine.persist import upsert_fact, Series
...
class ANInputs:
    ...
    # Base prices (c.Sales-AN)
    sales_price: float
    unit_cogs: float
    quantity_per_month: float

    # Revenue components
    freight_unit_price: float = 0.0
    mgmt_fee_unit_price: float = 0.0

    # Cost components
    freight_unit_cost: float = 0.0
...
def _get_an_base_prices(db: Session, scenario_id: int) -> Tuple[float, float, float, float, float]:
    """
    (sales_price, unit_cogs, freight_rev_u, mgmt_fee_u, freight_cost_u) döner.
    Şimdilik freight/mgmt fee/cost = 0; modellemeye açık tutuldu.
    """
    # 1) BOQ + kategori map
    row = db.execute(text("""
        WITH an_boq AS (
...
def _compute_rnf_multipliers(db: Session, policy_id: Optional[int], start_yyyymm: int, months: int, frequency: str="annual") -> List[float]:
    ...
    if frequency.lower().startswith("annual"):
        ...
        for i, _ in enumerate(months_list):
            block_start = (i // 12) * 12
            anchor_yyyymm = months_list[block_start]
            multipliers.append(basket_value(anchor_yyyymm))
        return multipliers

    return [basket_value(m) for m in months_list]


# --------------------------- Core computation -----------------------------

def compute_an_facts(
    db: Session,
    scenario_id: int,
    escalation_policy_id: Optional[int] = None,
    frequency: str = "annual",
) -> Tuple[List[FactRow], List[FactRow], List[FactRow]]:
    """
    Dönenler:
      - facts_m: c.Sales-AN (aylık komponentler) + o.base-AN (internal)
      - facts_q: oQ.Finance-AN (revenue/cogs/gp)
      - facts_a: oA.Finance-AN (revenue/cogs/gp)
    """
    ...
    if not escalation_policy_id:
        row = db.execute(
            text("SELECT default_price_escalation_policy_id AS pid FROM scenarios WHERE id=:sid"),
            {"sid": scenario_id}
        ).mappings().first()
    ...
    for i, yyyymm in enumerate(yyyymms):
        ...
        cogs_total    = cogs_ex_tax + freight_cost
        gp            = total_rev - cogs_total

        # c.Sales-AN (aylık breakdown)
        for series, val in [
        ...
        # Finans agregasyon için internal baz (tek isimler: revenue/cogs/gp)
        for series, val in [
            ("revenue", total_rev),
            ("cogs",    cogs_total),
            ("gp",      gp),
    ...
    def agg(series_filter: Iterable[str], period: str) -> List[FactRow]:
        out: List[FactRow] = []
        bucket: Dict[Tuple[int, int, str], float] = {}
        for fr in facts_m:
...
def persist_an_facts(
    db: Session,
    run_id: int,
    scenario_id: int,
    facts_m: List[FactRow],
    facts_q: List[FactRow],
    facts_a: List[FactRow],
) -> None:
    """
    (run_id, sheet_code, category_code, yyyymm, series) başına tek satır upsert eder.
    """
    if not _has_series_column(db):
        raise RuntimeError(
            "engine_facts_monthly tablosunda 'series' kolonu yok. "
            "Lütfen migration 20251016_add_series_to_engine_facts çalıştırın."
        )

    all_facts = facts_m + facts_q + facts_a
    bind = db.get_bind() or db.connection().engine
